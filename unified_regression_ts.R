...
#plink1.9 --bfile data_filtered --allow-no-sex --extract results/03/extract0000 --recodeA --out results/04/temp.1.BMI

#import argument
rawfile <- "results/04/temp.1.BMI.raw"
bimfile <- "results/03/extract0000"
phenfile <- "sibling-phenotypes.txt"
covfile <- "sibling-covariates-clean.txt"
outfile <- "results/04/output.1.BMI"
outcome <- "BMI"
...
bim <- fread(bimfile) #, sep = "" , header = F , nrows = 100, na.strings ="")#, stringsAsFactors= F)
...
for (i in 1:length(Variants)) {
	...
	ped2 <- data.table(FID=ped$FID, PHENOTYPE=ped$Outcome, GENOTYPE=as.numeric(unlist(ped[,snp_ind, with=F])),
			  FAM_MEAN=ave(as.numeric(unlist(ped[,snp_ind, with=F])), ped$FID, FUN=mean), AGE=ped$Age, SEX=ped$Sex,
			  PC1=ped$PC1, PC2=ped$PC2, PC3=ped$PC3, PC4=ped$PC4, PC5=ped$PC5, PC6=ped$PC6, PC7=ped$PC7, PC8=ped$PC8, PC9=ped$PC9,
			  PC10=ped$PC10, PC11=ped$PC11, PC12=ped$PC12, PC13=ped$PC13, PC14=ped$PC14, PC15=ped$PC15, PC16=ped$PC16, PC17=ped$PC17,
			  PC18=ped$PC18, PC19=ped$PC19, PC20=ped$PC20)

	# Centre genotype around family mean
	ped3 <- na.omit(ped2[,CENTREDGENOTYPE:=GENOTYPE-FAM_MEAN])

	# Extract total effect
	model1 <- lm(formula = PHENOTYPE ~ GENOTYPE + AGE + SEX + PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10+PC11+PC12+PC13+PC14+PC15+PC16+PC17+PC18+PC19+PC20,
		     data=ped3)
	...
	# Save the clustered SEs and corresponding P-values ofr WF/BF
...
#Exits R without storing the work space image
q("no")
